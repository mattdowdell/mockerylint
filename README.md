...
For a guide to contributing, see [CONTRIBUTING.md](./CONTRIBUTING.md), including guidance on the use
of generative AI.
...
them without writing to any files. Fixes are only suggested for unambiguous changes that do not cause
compilation errors. If a change would produce a compilation error, or there are multiple options for
resolution, no fix is suggested.
